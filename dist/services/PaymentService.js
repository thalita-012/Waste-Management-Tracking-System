...
const PaymentRepository_js_1 = require("../repositories/PaymentRepository.js");
...
const { BakongKHQR, IndividualInfo, MerchantInfo, khqrData } = requireFrom('bakong-khqr');
class PaymentService {
    constructor() {
        this.paymentRepo = new PaymentRepository_js_1.PaymentRepository();
    ...
    async createBakongPayment(orderId, amount, currency = 'KHR') {
        ...
        if (existingPayment) {
            if (existingPayment.status === 'PENDING') {
                logger_js_1.logger.info('Payment already exists for orderId, refreshing pending QR', { orderId });
            ...
            logger_js_1.logger.info('Payment already exists for orderId, returning existing non-pending payment', { orderId });
            return existingPayment;
        }
        logger_js_1.logger.info('Generating KHQR for new payment', { orderId, amount, currency });
        ...
        logger_js_1.logger.info('Payment created successfully', { orderId, paymentId: payment.id });
        return payment;
    }
    async verifyPayment(orderId, userId = 1) {
        const payment = await this.paymentRepo.findByOrderId(orderId);
        if (!payment) {
            logger_js_1.logger.warn('Payment not found', { orderId });
            throw new Error('Payment not found');
        }
        if (!payment.khqrMd5) {
            logger_js_1.logger.error('Payment has no KHQR MD5 value', { orderId, paymentId: payment.id });
            throw new Error('Payment does not have a KHQR MD5 value');
        }
        // Don't verify if already paid
        if (payment.status === 'PAID') {
            logger_js_1.logger.info('Payment already paid, skipping verification', { orderId });
        ...
        logger_js_1.logger.info('Checking transaction status with Bakong', { orderId });
        ...
        if (paid && payment.status === 'PENDING') {
            logger_js_1.logger.info('Marking payment as PAID', { orderId, transactionId });
    ...
    async getPaymentByOrderId(orderId) {
        ...
        if (payment.status === 'PENDING') {
            logger_js_1.logger.info('Refreshing pending QR before returning payment QR', { orderId });
            return await this.refreshPendingPaymentQr(payment);
        }
        return payment;
    }
    async checkConfiguredBakongAccount() {
        const baseUrl = (0, env_js_1.getRequiredConfig)('BAKONG_API_BASE_URL', env_js_1.env.bakong.apiBaseUrl).replace(/\/$/, '');
        const accountId = (0, env_js_1.getRequiredConfig)('BAKONG_ACCOUNT_ID', env_js_1.env.bakong.accountId);
    ...
    generateKhqr(orderId, amount, currency) {
        const bakongAccountId = (0, env_js_1.getRequiredConfig)('BAKONG_ACCOUNT_ID', env_js_1.env.bakong.accountId);
        const merchantName = this.limitKhqrText((0, env_js_1.getRequiredConfig)('BAKONG_MERCHANT_NAME', env_js_1.env.bakong.merchantName), 25);
        const merchantCity = this.limitKhqrText(env_js_1.env.bakong.merchantCity, 15);
        const currencyValue = currency === 'USD' ? khqrData.currency.usd : khqrData.currency.khr;
        const accountInformation = env_js_1.env.bakong.accountInformation;
        const merchantId = env_js_1.env.bakong.merchantId;
        const acquiringBank = env_js_1.env.bakong.acquiringBank;
        ...
        if (response.status.errorCode || !response.data) {
            logger_js_1.logger.error('KHQR generation failed', { orderId, error: response.status.message });
    ...
    async checkBakongTransaction(md5) {
        const baseUrl = (0, env_js_1.getRequiredConfig)('BAKONG_API_BASE_URL', env_js_1.env.bakong.apiBaseUrl).replace(/\/$/, '');
        const token = (0, env_js_1.getRequiredConfig)('BAKONG_API_TOKEN', env_js_1.env.bakong.apiToken);
        logger_js_1.logger.debug('Checking Bakong transaction', { md5 });
        try {
            ...
            if (!response.ok) {
                logger_js_1.logger.error('Bakong API error', {
                    md5,
            ...
            logger_js_1.logger.debug('Bakong transaction check successful', { md5, responseCode: data.responseCode });
            return data;
        }
        catch (error) {
            logger_js_1.logger.error('Bakong API request failed', { md5, error: error.message });
            throw error;
        }
    }
    ...
    getDynamicPaymentFields(amount) {
        return {
            amount,
            expirationTimestamp: Date.now() + env_js_1.env.bakong.qrExpirationMinutes * 60 * 1000,
        };
    }
}
